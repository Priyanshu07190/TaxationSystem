...
public class AdminFrame {
    ...
    public void launch() {
        JFrame frame = new JFrame("Taxation System - Admin");
        frame.setSize(600, 500);
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setLayout(null);

        // Common Salaried User Functionalities
        ...
        chatbotButton.setBounds(50, 350, 150, 30);
        frame.add(chatbotButton);

        JButton linkAadharButton = new JButton("Link Aadhaar");
        linkAadharButton.setBounds(50, 390, 150, 30);
        frame.add(linkAadharButton);

        // Admin-Specific Functionalities
        JButton searchDocButton = new JButton("Search Documents");
        searchDocButton.setBounds(250, 30, 150, 30);
        frame.add(searchDocButton);

        JButton activityButton = new JButton("View All Activities");
        activityButton.setBounds(250, 70, 150, 30);
        frame.add(activityButton);

        JButton verifyPanButton = new JButton("Verify PAN");
        verifyPanButton.setBounds(250, 110, 150, 30);
        frame.add(verifyPanButton);

        JButton updateTaxSlabButton = new JButton("Update Tax Slab");
        updateTaxSlabButton.setBounds(250, 150, 150, 30);
        frame.add(updateTaxSlabButton);

        JButton updateUserButton = new JButton("Update User");
        updateUserButton.setBounds(250, 190, 150, 30);
        frame.add(updateUserButton);

        JButton deleteUserButton = new JButton("Delete User");
        deleteUserButton.setBounds(250, 230, 150, 30);
        ...
        calcTaxButton.addActionListener(e -> {
            frame.dispose();
            new TaxCalculatorFrame(userId, "Salaried", "Admin").launch();
        });
        ...
        quizButton.addActionListener(e -> {
            try {
                ...
                frame.dispose();
                new QuizFrame(userId, "Salaried", "Admin").launch();
            } catch (Exception ex) {
        ...
        searchDocButton.addActionListener(e -> {
            frame.dispose();
            new SearchDocumentsFrame(userId).launch();
        });

        activityButton.addActionListener(e -> {
            frame.dispose();
            new UserActivityFrame(userId, true).launch();
        });
        ...
        updateTaxSlabButton.addActionListener(e -> {
            frame.dispose();
            new UpdateTaxSlabFrame(userId).launch();
        });

        updateUserButton.addActionListener(e -> {
            frame.dispose();
            new UpdateUserFrame(userId).launch();
        });

        deleteUserButton.addActionListener(e -> {
            String userIdStr = JOptionPane.showInputDialog(frame, "Enter User ID to delete:");
            if (userIdStr != null) {
                try {
                    int targetUserId = Integer.parseInt(userIdStr);
                    dbManager.deleteUser(userId, targetUserId);
